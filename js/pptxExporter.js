...
export function setupPPTXExport({ 
  buttonId = "exportPPTX", 
  ...
  freshBtn.addEventListener("click", () => {
    ...
    if (!diagramPanel) {
      ...
      exportToPowerPoint(liveResult, liveRequirements, null);
      return;
    }
...
function prepareDiagramForCapture(diagramPanel, liveResult, liveRequirements) {
  ...
  console.log("🔧 Starting diagram preparation...");
...
function refreshDiagramVisuals() {
  ...
  return new Promise((resolve) => {
    try {
      console.log("🔄 Refreshing diagram visuals...");
...
function waitForDiagramReady(diagramPanel) {
  return new Promise((resolve) => {
    ...
    const attemptDelay = 100; // Increased delay
    ...
    function checkDiagram() {
      ...
      if (bounds.width >= 100 && bounds.height >= 100) {
        // ✅ Additional check for child elements AND their computed styles
      ...
      // ✅ Re-trigger rendering on failed attempts
      if (attempts % 2 === 0) {
...
function checkForVisibleChildren(diagramPanel) {
  try {
    // ✅ Check if the diagram has actual content elements
...
function checkForRenderedContent(diagramPanel) {
  try {
    // ✅ Check for SVG content with actual paths/shapes
    ...
    svgElements.forEach(svg => {
      ...
      if (paths.length > 0) {
        // Check if any path has actual content (not empty or default)
        paths.forEach(path => {
    ...
    // ✅ Also check for positioned divs (node elements)
    ...
    return true; // Assume content exists if we can't check
  }
}
...
function captureDiagramImage(diagramPanel, liveResult, liveRequirements, wasHidden) {
  ...
  scale: 2
      ...
      .then((canvas) => {
        const DPI = 96; // Match this to your export logic
...
const bounds = diagramPanel.getBoundingClientRect();
console.log("📐 Original DOM bounds (px):", bounds.width, bounds.height);
        ...
        if (wasHidden) {
         ...
         exportToPowerPoint(liveResult, liveRequirements, diagramImage);
      ...
      // ✅ Still export without diagram
      exportToPowerPoint(liveResult, liveRequirements, null, bounds);
      resolve();
    }
  });
}